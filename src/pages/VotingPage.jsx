...
const VotingPage = () => {
  ...
  const { isExpired } = useContext(CountdownContext);
          ...
          <div className="grid gap-3 sm:gap-6 [grid-template-columns:repeat(2,2fr)] sm:[grid-template-columns:repeat(auto-fit,minmax(180px,1fr))] xl:[grid-template-columns:repeat(auto-fit,minmax(250px,1fr))] px-1 sm:px-4">
            {teams.map((team) => (
              ...
              >
                ...
                <button
                  disabled={isExpired}
                  className="bg-yellow cursor-pointer disabled:bg-yellow-200 disabled:cursor-not-allowed text-black px-4 py-2 rounded-full font-semibold hover:bg-yellow-400 transition-colors"
                  onClick={() =>
                    handleVote(team.id, {
